...
def test_hla_returns_structured_result():
    ...
    assert "matched_markers" in result and "total_markers" in result and "details" in result

def test_matching_never_returns_blood_incompatible_candidate():
    donor = {"id": 1, "blood_group":"A+", "organs_available":["kidney"], "availability_status":"ACTIVE"}
    recipient = {"id": 2, "blood_group":"B+", "required_organ":"kidney", "urgency":"CRITICAL", "eligible":True}
...
def test_matching_returns_ranked_multiple_candidates():
    ...
    donors = [
        {"id":1,"blood_group":"O+","organs_available":["kidney"],"availability_status":"ACTIVE","hla_a":"A1"},
        {"id":2,"blood_group":"O+","organs_available":["kidney"],"availability_status":"ACTIVE","hla_a":"A2"},
    ]
...
def test_urgency_is_transparent():
    ...
    assert result["factors"]["urgency"] == "EMERGENCY"
