...
def create_patient(body: PatientIn, user: User = Depends(require_role("doctor")), db: Session = Depends(get_db)):
    ...
    if not doctor or doctor.approval_status != "approved":
        raise HTTPException(403, "Only an organizer-approved doctor can register a patient.")
    if not doctor.hospital_id:
        raise HTTPException(400, "Your doctor profile isn't linked to a hospital yet.")

    patient = Patient(hospital_id=doctor.hospital_id, doctor_id=doctor.id, **body.dict())
    db.add(patient)
    db.commit()
    db.refresh(patient)
    log_action(db, "PATIENT_CREATED", user_id=user.id, target=patient.id)
...
def list_my_patients(user: User = Depends(require_role("doctor")), db: Session = Depends(get_db)):
    doctor = db.query(DoctorProfile).filter(DoctorProfile.user_id == user.id).first()
    if not doctor:
        raise HTTPException(404, "No doctor profile yet.")
    patients = db.query(Patient).filter(Patient.doctor_id == doctor.id).all()
    for p in patients:
        p.priority_score = calculate_priority(p.urgency, p.waiting_since)["score"]
    db.commit()
    return to_dict_list(patients)
