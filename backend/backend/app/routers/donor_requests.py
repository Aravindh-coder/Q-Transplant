...
from app.models import User, DonorProfile, Patient, DonorRequest, Notification
...
def _request_view(req: DonorRequest, requester: User, donor: DonorProfile):
    return {
        ...
        "requested_by": {
            "name": requester.full_name or requester.email,
            "role": requester.role,
        },
        "donor_id": donor.id,
    }
...
def create_request(body: CreateRequestIn,
                   user: User = Depends(require_role("doctor", "hospital")),
    ...
    if body.patient_id:
        ...
        if user.role == "doctor" and patient.doctor_id:
            # Doctor-specific patient ownership is enforced through the profile owner.
            doctor_profile = next((getattr(user, "doctor_profile", None),), None)
            # The patient ownership check is completed below using the doctor profile query.
    ...
    req = DonorRequest(donor_id=donor.id, patient_id=body.patient_id, requested_by=user.id,
                       organ=body.organ, urgency=urgency, message=body.message, status="pending")
    db.add(req)
    db.flush()
    notification = Notification(recipient_user_id=donor.user_id, type="IN_APP",
                                 title=f"New {body.organ} donation request",
                                 message=(f"{user.full_name or user.email} ({user.role}) sent you a "
                                          f"{urgency} donation request. Open your donor requests to respond."),
    ...
    if donor_user:
        try:
            send_email(donor_user.email, f"Q-Transplant — {urgency} donor request",
                       f"You have a new {body.organ} donation request from {user.full_name or user.email} ({user.role}). "
                       "Log in to Q-Transplant to review and respond.")
        except RuntimeError:
            pass
    log_action(db, "DONOR_REQUEST_CREATED", user_id=user.id, target=req.id,
               meta={"donor_id": donor.id, "urgency": urgency, "organ": body.organ})
...
def my_requests(user: User = Depends(require_role("donor")), db: Session = Depends(get_db)):
    ...
    rows = (db.query(DonorRequest, User)
            .join(User, User.id == DonorRequest.requested_by)
            .filter(DonorRequest.donor_id == donor.id)
            .order_by(DonorRequest.created_at.desc()).all())
    return [_request_view(req, requester, donor) for req, requester in rows]


@router.get("/{request_id}")
def get_request(request_id: str, user: User = Depends(require_role("donor", "doctor", "hospital", "organizer")),
               db: Session = Depends(get_db)):
...
def respond(request_id: str, body: RespondIn, user: User = Depends(require_role("donor")), db: Session = Depends(get_db)):
    ...
    db.add(Notification(recipient_user_id=req.requested_by, type="IN_APP",
                         ...
                         message=f"The donor has {decision} your {req.organ} request.",
                         priority="high"))
    db.commit()
    requester = db.query(User).filter(User.id == req.requested_by).first()
    if requester:
        try:
            ...
            pass
    log_action(db, "DONOR_REQUEST_RESPONDED", user_id=user.id, target=req.id,
               meta={"decision": decision})
    return {"request_id": req.id, "status": req.status, "donation_status": donor.donation_status}


@router.post("/{request_id}/cancel")
def cancel(request_id: str, body: CancelIn, user: User = Depends(require_role("doctor", "hospital", "organizer")),
           db: Session = Depends(get_db)):
    req = db.query(DonorRequest).filter(DonorRequest.id == request_id).first()
    if not req:
        raise HTTPException(404, "Request not found.")
    ...
    db.add(Notification(recipient_user_id=req.donor_id, type="IN_APP", title="Donation request cancelled",
