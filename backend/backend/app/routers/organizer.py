...
def _doctor_review(db, doctor):
    ...
    docs=db.query(Document).filter(Document.owner_user_id==doctor.user_id).all()
...
def pending_doctors(user=Depends(require_role("organizer")),db:Session=Depends(get_db)):
    doctors=db.query(DoctorProfile).filter(DoctorProfile.approval_status.in_(["pending","PENDING_APPROVAL","ORGANIZER_REVIEW","REQUESTED_INFO"])).all()
    return [_doctor_review(db,d) for d in doctors]

@router.get("/doctors/{doctor_id}/review")
def review_doctor(doctor_id:str,user=Depends(require_role("organizer")),db:Session=Depends(get_db)):
...
def approve_doctor(doctor_id:str,user=Depends(require_role("organizer")),db:Session=Depends(get_db)):
    d=db.query(DoctorProfile).filter(DoctorProfile.id==doctor_id).first()
    ...
    notify(db,account,"Q-Transplant — you're approved","Your doctor account has been approved. You can now access the doctor portal.",priority="high",also_email=True)
    log_action(db,"DOCTOR_APPROVED",user_id=user.id,target=doctor_id); return {"id":d.id,"approval_status":d.approval_status}

@router.post("/doctors/{doctor_id}/reject")
def reject_doctor(doctor_id:str,reason:str="",user=Depends(require_role("organizer")),db:Session=Depends(get_db)):
    d=db.query(DoctorProfile).filter(DoctorProfile.id==doctor_id).first()
    if not d: raise HTTPException(404,"Doctor profile not found.")
    d.approval_status="REJECTED"; account=db.query(User).filter(User.id==d.user_id).first(); account.status="rejected"; db.commit()
    notify(db,account,"Q-Transplant — registration update",f"Your doctor registration was rejected.{(' Reason: '+reason) if reason else ''}",priority="high",also_email=True)
    log_action(db,"DOCTOR_REJECTED",user_id=user.id,target=doctor_id,meta={"reason":reason}); return {"id":d.id,"approval_status":d.approval_status}

@router.post("/doctors/{doctor_id}/request-information")
def request_doctor_information(doctor_id:str,message:str="Please provide the requested additional information.",user=Depends(require_role("organizer")),db:Session=Depends(get_db)):
    d=db.query(DoctorProfile).filter(DoctorProfile.id==doctor_id).first()
    if not d: raise HTTPException(404,"Doctor profile not found")
    d.approval_status="REQUESTED_INFO"; account=db.query(User).filter(User.id==d.user_id).first(); db.commit()
    notify(db,account,"Additional information required",message,priority="high",also_email=True); log_action(db,"DOCTOR_INFO_REQUESTED",user_id=user.id,target=doctor_id,meta={"message":message})
    return {"id":doctor_id,"approval_status":d.approval_status}

@router.post("/hospitals/{hospital_id}/verify")
def verify_hospital(hospital_id:str,user=Depends(require_role("organizer")),db:Session=Depends(get_db)):
    ...
    h.verification_status="verified"; account=db.query(User).filter(User.id==h.user_id).first(); account.status="active"; db.commit(); log_action(db,"HOSPITAL_VERIFIED",user_id=user.id,target=hospital_id); return {"id":h.id,"verification_status":h.verification_status}

@router.get("/users")
def users(role:str|None=None,user=Depends(require_role("organizer")),db:Session=Depends(get_db)):
    q=db.query(User)
...
def suspend_user(target_user_id:str,user=Depends(require_role("organizer")),db:Session=Depends(get_db)):
    target=db.query(User).filter(User.id==target_user_id).first()
    if not target: raise HTTPException(404,"User not found.")
    target.status="suspended"; db.commit(); log_action(db,"ACCOUNT_SUSPENDED",user_id=user.id,target=target_user_id); return {"id":target.id,"status":target.status}

@router.post("/users/{target_user_id}/reactivate")
def reactivate_user(target_user_id:str,user=Depends(require_role("organizer")),db:Session=Depends(get_db)):
    target=db.query(User).filter(User.id==target_user_id).first()
    if not target: raise HTTPException(404,"User not found.")
    target.status="active"; db.commit(); log_action(db,"ACCOUNT_REACTIVATED",user_id=user.id,target=target_user_id); return {"id":target.id,"status":target.status}
